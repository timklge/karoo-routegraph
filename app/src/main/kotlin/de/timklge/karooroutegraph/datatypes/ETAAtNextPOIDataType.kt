            ...
            viewModelProvider.viewModelFlow.collect { state ->
                ...
                if (nextPoi != null){
                    travelTimeEstimationService.estimateTravelTime(
                        ...
                        endDistance = nextPoi.second.distanceFromRouteStart.toDouble(),
                        totalWeight = TODO(),
                        lastHourAvgPower = TODO(),
                        lastHourAvgSpeed = TODO(),
                        surfaceConditions = TODO()
                    )

                    val nextPoiDistance = nextPoi.second.distanceFromRouteStart - currentDistanceAlongRoute
                    emitter.onNext(StreamState.Streaming(DataPoint(dataTypeId, mapOf(DataType.Field.SINGLE to nextPoiDistance.toDouble()))))
                } else {
                    emitter.onNext(StreamState.NotAvailable)
                }
            }
